...
def process_battery_data(current_battery, battery_type, serial_number, engine):
    ...
    with open(f'measured_data_pkl_files\{battery_type}_{serial_number}_SBS.pkl', 'wb') as file:
    ...
    current_battery.data_SBS.to_sql(f'{battery_type}_{serial_number}_SBS', engine, if_exists='replace')
    ...
    with open(f'measured_data_pkl_files\{battery_type}_{serial_number}_dataflash.pkl', 'wb') as file:
    ...
    current_battery.data_df.to_sql(f'{battery_type}_{serial_number}_dataflash', engine, if_exists='replace')
    print(f"Done sending dataflash for {battery_type} to server...")
    print("Add comparison to golden file later...")


def read_battery_data(current_battery, battery_type, serial_number):
    """Read battery data and save it to pickle files and SQL database."""

    if battery_type == "bq3060":
        battery_type = "BQ3060"
        process_battery_data(current_battery, battery_type, serial_number, engine)
...
def gui_to_read_battery(scanned_input):
    try:
        ...
        read_battery_data(battery, battery_type, serial_number)
